    ...
    type: mongoose.Schema.Types.ObjectId`;
		...
		if (fieldConnections.length > 0 && currentCollection) {
			...
			if (refCollectionId && refCollectionId !== currentCollection.id) {
				const refCollection = collections.find(
					(col) => col.id === refCollectionId
				);
				if (refCollection) {
					fieldDef += `,
    ref: '${refCollection.name}'`;
		...
		const arrayType =
			MONGOOSE_TYPE_MAPPING[
				field.arrayType as keyof typeof MONGOOSE_TYPE_MAPPING
			] || 'String';
		fieldDef = `[${arrayType}]`;
	} else {
		const mongooseType =
			MONGOOSE_TYPE_MAPPING[type as keyof typeof MONGOOSE_TYPE_MAPPING] ||
			'String';
		fieldDef = `{
    type: ${mongooseType}`;
	}
...
export function generateMongooseSchema(
	...
	const fieldDefinitions = regularFields
		.map((field, fieldIndex) => {
			...
			const fieldConnections = connections.filter(
				(conn) =>
					(conn.sourceCollectionId === collection.id &&
						conn.sourceFieldIndex === originalFieldIndex) ||
					(conn.targetCollectionId === collection.id &&
						conn.targetFieldIndex === originalFieldIndex)
			);

			return generateFieldDefinition(
				field,
				allCollections,
				fieldConnections,
				collection
